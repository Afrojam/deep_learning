...
plots_folder = Path("plots/fine_tune_fnn")
models_folder = Path("models/")
dataset_name = "cifar"
original_model_name = 'fnn_{}_fine_tuned_test'.format(dataset_name)
imgs_folder = Path("imgs/")
text_file = "CIFAR-FNN score file_test.txt"
    ...
    for (optimizer, optimizer_name) in zip([keras.optimizers.SGD(),
        ...
        for kernel_initializer in kernel_initializer_list:
            ...
            for layers in range(n_hlayers):
                neurons_accuracy = []
                for neurons in range(initial_neurons, n_neurons, neurons_step):
                    nn = Sequential()
                    nn.add(Dense(initial_layer_neurons, activation='relu', input_shape=x_train.shape[1:],
                    ...
                    model_name = model_name + '_{}_hidden_layers'.format(layers + 1)
                ...
                do_plot(plots_folder / 'model_neurons_acc_{0}.png'.format(model_name), [neurons_accuracy],
                        'model neurons acc',
                        'number of neurons (x10)', 'accuracy', ['acc'])
