...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        output = self.attn_combine(output).unsqueeze(0) #Returns a new tensor with an added dimension to the original tensor.
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    with torch.no_grad():
        ...
        for di in range(max_length):
            ...
            decoder_input = topi.squeeze().detach()  # take output as input, detach from history

        return decoded_words  # returns outputs
